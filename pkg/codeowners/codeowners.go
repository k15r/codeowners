// based on https://github.com/hairyhenderson/go-codeowners
package codeowners
...
// Groups - return a list of Codeowner-Objects.
// patterns that matched with their owners for a given list of files
func (c *Codeowners) Groups(paths, alreadyapproved []string) []Codeowner {
	g := map[string][]string{}
		...
		for i := len(c.patterns) - 1; i >= 0; i-- {
			...
			if p.re.MatchString(path) {
				for _, owner := range p.owners {
					for _, approver := range alreadyapproved {
						if owner == approver {
							continue PATTERN
						}
					}
				}
				g[p.pattern] = p.owners
			}
		}
	}
	groups := []Codeowner{}
	for pattern, owners := range g {
		groups = append(groups, Codeowner{
			pattern: pattern,
			owners:  owners,
		})
	}
